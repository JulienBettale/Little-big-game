...
TileMap::TileMap(float gridSize, int width, int height, std::string texture_file)
{
	...
	if (!this->tileSheet.loadFromFile(texture_file))
		std::cout << "ERROR::TILEMAP::TILETEXTURESHEET NON GARGEE!::FILENAME: " << texture_file << "\n";
...
{
	//Suppréssions des tuiles sur x, y et z si c'est possible
	if (x < this->maxSizeWorldGrid.x && x >= 0 &&
		y < this->maxSizeWorldGrid.y && y >= 0 &&
		z < this->layers && z >= 0)
	{
...
{
	/* Sauvegarde de l'ensemble des tuiles de la map dans un text-file.
-       Format :
-       Size x et y
-       GridSize
-       Layers
...
void TileMap::loadFromFile(const std::string file_name)
{
	...
	{
		std::cout << "ERROR::TILEMAP::CHARGEMENT DU FICHIER MAP::FILENAME: " << file_name << "\n";
	}
